...
final class MapListRestWorker: MapListRestWorkerProtocol {
    func fetch(completion: @escaping RestResponse<[RouteRestModel]>) {
        AF.request("https://sandbox-giravolta-static.s3.eu-west-1.amazonaws.com/tech-test/trips.json").responseData { response in
            ...
            case .success(let result):
                guard let listModel = try? JSONDecoder().decode([RouteRestModel].self, from: result) else {
